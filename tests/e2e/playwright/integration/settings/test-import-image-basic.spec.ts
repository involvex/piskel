...
test("Test importing a simple image", async ({ page }) => {
  ...
  await page.waitForSelector(".palettes-list-color:nth-child(1)", {
    state: "attached",
  });
  ...
  await expect(
    page.locator(".current-step.import-image-container"),
  ).toBeAttached();
  // Check for import and replacing as a single image.
  await expect(
    page.locator('input[name="import-type"][value="single"]'),
  ).toBeChecked();
  ...
  await page.waitForSelector(".current-step .import-resize-info", {
    state: "attached",
  });
  ...
  await page.waitForSelector(".current-step .insert-mode-container", {
    state: "attached",
  });

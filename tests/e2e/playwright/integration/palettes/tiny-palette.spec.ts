...
test("Test palette switches to tiny mode if it contains more than 10 colors", async ({
  page,
}) => {
  await openEditor(page);

  await setPiskelFromImageSrc(page, src_4_colors);
  await page.waitForSelector(".palettes-list-color:nth-child(4)", {
    state: "attached",
  });
  await expect.poll(() => getPaletteColorCount(page)).toBe(4);
  await page.waitForSelector(".palettes-list-colors:not(.tiny)", {
    state: "visible",
  });

  await setPiskelFromImageSrc(page, src_16_colors);
  await page.waitForSelector(".palettes-list-colors.tiny", {
    state: "attached",
  });

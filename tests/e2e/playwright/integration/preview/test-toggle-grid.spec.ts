...
const isGridEnabled = async (page: Page): Promise<boolean> => {
  return await page.evaluate(() => {
    return window.pskl.UserSettings.get(
      window.pskl.UserSettings.GRID_ENABLED,
    ) as boolean;
  });
};

test("Test toggling the grid using the animated preview toggle grid icon", async ({
  page,
}) => {
  ...
  await expectHasNotClass(
    page,
    GRID_BUTTON_SELECTOR,
    INACTIVE_GRID_BUTTON_CLASS,
  );
